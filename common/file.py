...
# 標準ライブラリのインポート
import json,csv,os,shutil,datetime,time,platform,zipfile,tarfile,hashlib,copy,base64,itertools,pathlib,PIL,io
...
class DirData():
    ...
    def dir_name(self):
        ...
        if len(dir_elms)>0:
            ...
            dn = f"{DirData._DEFAULT_NAME}{self.sep}"
        return dn
...
class CryptFileData(FileData):
    ...
    def __init__(self,file_path="",key_path="",encoding="utf-8",decoding="ascii",newline=None,first_read=True):
        ...
        self.key = FileData(key_path,encoding,newline,first_read)
        self.decoding = decoding
